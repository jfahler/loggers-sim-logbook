  ...
  async (req) => {
    ...
    if (!pilot) {
      pilot = await logbookDB.queryRow<{ id: number }>`
        ...
        VALUES (${flightData.pilotName}, ${flightData.callsign})
        RETURNING id
      `;
    }
    ...
    const flight = await logbookDB.queryRow<{ id: number }>`
      INSERT INTO flights (
        ...
        ${pilot!.id}, ${flightData.aircraftType}, ${flightData.missionName},
        ${flightData.startTime}, ${flightData.endTime}, ${flightData.durationSeconds},
        ${flightData.maxAltitudeFeet}, ${flightData.maxSpeedKnots}, ${flightData.distanceNm},
    ...
    for (const event of flightData.events) {
      await logbookDB.exec`
        INSERT INTO flight_events (
          ...
          ${event.description}, ${event.targetName}, ${event.weaponUsed}
        )
      `;
    }
      ...
      for (const part of parts) {
        if (part.startsWith('T=')) {
          ...
          if (coords.length >= 3) {
            ...
            const alt = parseFloat(coords[2]) * 3.28084; // Convert meters to feet
        ...
        if (part.startsWith('Name=')) {
          ...
          if (name.includes('|')) {
            ...
            flightData.pilotName = nameParts[0];
            if (nameParts.length > 1) {
              flightData.callsign = nameParts[1];
            }
          } else {
            flightData.pilotName = name;
          }
